...
class Yanka_Dummy_Import {
	...
	private function _set_home_page() {
		...
		if($this->_page == 'base') {
			...
			$home_page_title = $this->_yanka_pages[$this->_page]['title'];
		}
	...
	private function _import_theme_options() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			if( $options_file ) {
				...
				ob_end_clean();
				try {
					update_option( 'yanka_option', $theme_options );
	...
	private function _load_importers() {
		...
		$path = apply_filters('yanka_require', YANKA_ADDONS_PATH . '/classes/wordpress-importer.php');
	...
	private function _is_valid_page( $page ) {
		if( in_array($page, array_keys( $this->_yanka_pages ) )) return true;
		return false;
	}
