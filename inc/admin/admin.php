...
class Yanka_Theme_Admin {
	...
	function yanka_admin_script_local() {
		$localize_data = array(
		...
		$localize_data['searchOptionsPlaceholder'] = esc_js(esc_html__('Search for options', 'yanka'));
		return apply_filters( 'moros_admin_script_local', $localize_data );
	}

	public function moros_admin_scripts_localize() {
		wp_localize_script( 'yanka-admin-js', 'yankaConfig', $this->yanka_admin_script_local() );
	}

	public function admin_enqueue_scripts( $hook ) {
		wp_enqueue_script( 'yanka-admin-js', get_template_directory_uri() . '/assets/js/admin.js', array(), false, true );
    ...
    public function activation_redirect() {
        if ( current_user_can( 'edit_theme_options' ) ) {
            header( 'Location:' . admin_url() . 'admin.php?page=yanka' );
        }
    }
    ...
    public function admin_init() {
        ...
        if ( current_user_can( 'edit_theme_options' ) ) {
            if ( isset( $_GET['yanka-deactivate'] ) && 'deactivate-plugin' == $_GET['yanka-deactivate'] ) {
                check_admin_referer( 'yanka-deactivate', 'yanka-deactivate-nonce' );
            ...
            } if ( isset( $_GET['yanka-activate'] ) && 'activate-plugin' == $_GET['yanka-activate'] ) {
                check_admin_referer( 'yanka-activate', 'yanka-activate-nonce' );
                ...
                foreach ( $plugins as $plugin ) {
                    if ( isset( $_GET['plugin'] ) && $plugin['slug'] == $_GET['plugin'] ) {
                        ...
                        wp_redirect( admin_url( 'admin.php?page=yanka-plugins' ) );
                        exit;
                    }
                }
            }
    ...
    public function admin_menu(){
        if ( current_user_can( 'edit_theme_options' ) ) {
            ...
            $welcome_screen = $add_menu( 'Yanka', 'Yanka', 'administrator', 'yanka', array( $this, 'welcome_screen' ), 'dashicons-image-filter', 59 );

            $sub_menu = 'add_submenu_' . 'page';
            $welcome       = $sub_menu( 'yanka', esc_html__( 'Welcome', 'yanka' ), esc_html__( 'Welcome', 'yanka' ), 'administrator', 'yanka', array( $this, 'welcome_screen' ) );
            $plugins       = $sub_menu( 'yanka', esc_html__( 'Plugins', 'yanka' ), esc_html__( 'Plugins', 'yanka' ), 'administrator', 'yanka-plugins', array( $this, 'plugins_tab' ) );
            $samples       = $sub_menu( 'yanka', esc_html__( 'Install Samples', 'yanka' ), esc_html__( 'Install Samples', 'yanka' ), 'administrator', 'yanka-samples', array( $this, 'samples_tab' ) );
        }
    }
    ...
    public function plugins_tab() {
        require_once( get_template_directory() . '/inc/admin/admin_pages/yanka-plugins.php' );
    }
    ...
    public function plugin_link( $item ) {
        ...
        if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
            $actions = array(
                'install' => sprintf(
                    '<a href="%1$s" class="button button-primary" title="'. esc_attr__('Install', 'yanka') .' %2$s">'. esc_html__('Install', 'yanka') .'</a>',
        ...
        elseif ( !yanka_plugin_active($item['plg_class'], $item['plg_func']) ) {
            $actions = array(
                'activate' => sprintf(
                    '<a href="%1$s" class="button button-primary" title="'. esc_attr__('Activate', 'yanka') .' %2$s">'. esc_html__('Activate', 'yanka') .'</a>',
                    esc_url( add_query_arg(
                        array(
                            'plugin'               => urlencode( $item['slug'] ),
                            'plugin_name'          => urlencode( $item['sanitized_plugin'] ),
                            'plugin_source'        => urlencode( $item['source'] ),
                            'yanka-activate'       => 'activate-plugin',
                            'yanka-activate-nonce' => wp_create_nonce( 'yanka-activate' ),
                        ),
                        admin_url( 'admin.php?page=yanka-plugins' )
                    ) ),
                    $item['sanitized_plugin']
                ),
            );
        ...
        elseif ( version_compare( $installed_plugins[$item['file_path']]['Version'], $item['version'], '<' ) ) {
            $actions = array(
                'update' => sprintf(
                    '<a href="%1$s" class="button button-primary" title="'. esc_attr__('Update', 'yanka') .' %2$s">'. esc_html__('Update', 'yanka') .'</a>',
                    wp_nonce_url(
                        add_query_arg(
                            array(
                                'page'          => urlencode( TGM_Plugin_Activation::$instance->menu ),
            ...
            $actions = array(
                'deactivate' => sprintf(
                    '<a href="%1$s" class="button button-primary" title="'. esc_attr__('Deactivate', 'yanka') .' %2$s">'. esc_html__('Deactivate', 'yanka') .'</a>',
                    esc_url( add_query_arg(
                        array(
                            'plugin'                 => urlencode( $item['slug'] ),
                            'plugin_name'            => urlencode( $item['sanitized_plugin'] ),
                            'plugin_source'          => urlencode( $item['source'] ),
                            'yanka-deactivate'       => 'deactivate-plugin',
                            'yanka-deactivate-nonce' => wp_create_nonce( 'yanka-deactivate' ),
                        ),
                        admin_url( 'admin.php?page=yanka-plugins' )
                    ) ),
                    $item['sanitized_plugin']
...
$main_import = new Yanka_Main_Import();
