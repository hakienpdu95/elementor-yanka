...
class Yanka_Main_Import {

	private $_yanka_pages = array();
	...
	public function __construct() {
		$this->_yanka_pages = yanka_get_config();
	...
	public function data_import_box( $type = false ) {
		$btn_label = esc_html__( 'Import Home', 'yanka' );
		$activate_label = '';
		...
		?>
			<div class="wrap metabox-holder yanka-import-page">

				<?php if ( ! function_exists( 'is_shop' ) ): ?>
					<p class="yanka-notice">
						<?php printf(esc_html__('To import data properly we recommend you to install', 'yanka'));?> <strong><?php printf('<a href="%s">WooCommerce</a>', esc_url( add_query_arg( 'page', urlencode( 'tgmpa-install-plugins' ), self_admin_url( 'themes.php' ) ) ));?></strong> <?php  printf(esc_html__('plugin', 'yanka'));?>
					</p>
				<?php endif ?>

				<?php if ( !$shop_page ): ?>
					<p class="yanka-warning">
						<?php
							esc_html_e( 'It seems that you didn\'t run WooCommerce setup wizard or didn\'t create a shop and the import can\'t be run now. You need to run WooCommerce setup wizard or install pages manually via WooCommerce -> System status -> Tools.', 'yanka' );
						?>
					</p>
				<?php endif ?>
				...
				<?php if( $this->_required_plugins() ): ?>
					<p class="yanka-warning">
						<?php printf(esc_html__('You need to install the following plugins to use our import function:','yanka'));?> <strong><?php printf('<a href="%s">%s</a>', esc_url( add_query_arg( 'page', urlencode( 'tgmpa-install-plugins' ), self_admin_url( 'themes.php' ) ) ), implode(', ', $this->_required_plugins()));
						?></strong>
					</p>
				<?php endif; ?>

				<form action="#" method="post" class="yanka-import-form">

					<div class="import-response"></div>
					<div class="yanka-import-progress animated" data-progress="0">
							<div class="width-0"></div>
					</div>
					<?php if ( $type == 'base' ): ?>
						<?php
							$btn_label = esc_html__( 'Import Base Data', 'yanka' );
							$activate_label = esc_html__( 'Activate Base Data', 'yanka' );

							if( $this->is_page_imported('base') ) $btn_label = $activate_label;

							$this->page_preview();

							$list = $this->_yanka_pages;
						...
						<input type="hidden" class="yanka_pages" name="yanka_pages" value="<?php echo esc_attr( $all ); ?>">
						<?php if( ! $this->is_page_imported('base') ): ?>
							...
							<div class="full-import-box">
								<label for="full_import">
									...
									<?php esc_html_e('Include all pages', 'yanka'); ?>
								</label>
							</div>
					...
					<?php else: ?>
						<?php
							...
							if( $type == 'page' ) $btn_label = esc_attr__( 'Import Page', 'yanka' );
	...
	public function pages_select( $type = false ) {
		...
		$list = $this->_yanka_pages;
		if( $type ) {
			$list = array_filter( $this->_yanka_pages, function( $el ) use($type) {
				return $type == $el['type'];
			});
		...
		?>
			...
			<select name="page_select" class="page_select">
				<option><?php esc_html_e('--Select--', 'yanka'); ?></option>
				<?php foreach ($list as $key => $value): ?>
					<option value="<?php echo esc_attr( $key ); ?>" data-imported="<?php if($this->is_page_imported( $key )){
							echo esc_html__('yes', 'yanka');
						}else{
							echo esc_html__('no', 'yanka');
	...
	public function page_preview( $page = 'base' ) {
		?>
			<div class="page-preview">
				<img src="<?php echo YANKA_URL; ?>/inc/admin/data/<?php echo esc_attr( $page ); ?>/preview.jpg" data-dir="<?php echo YANKA_URL; ?>/inc/admin/data" alt="<?php esc_attr_e('Page preview', 'yanka'); ?>" />
			</div>
		<?php
	}
	...
	public function import_action() {
		...
		foreach ($pages as $page) {
			...
			$this->_process = explode(',', $this->_yanka_pages[$this->_page]['process']);

			$type = $this->_yanka_pages[$this->_page]['type'];
			...
			$import = new Yanka_Dummy_Import( $this->_page, $this->_process );
			$import->run_import();
	...
	public function imported_pages() {
		$data = get_option('yanka_imported_pages');
	...
	public function add_imported_page( $page = false ) {
		...
		$imported[] = $page;
		return update_option( 'yanka_imported_pages', $imported );
	}
	...
	public function clean_imported_page_data(){
		return delete_option( 'yanka_imported_pages' );
	}
	...
	public function import_attributes() {
		if ( get_option( 'yanka_import_attributes' ) == true ) return;

		$import_attributes = $this->create_attributes();

		if ( $import_attributes ) {
			update_option( 'yanka_import_attributes', true );
		}
	}

	public function clean_import_attributes_data(){
		return delete_option( 'yanka_import_attributes' );
	}
