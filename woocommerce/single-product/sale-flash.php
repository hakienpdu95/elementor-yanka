...
// Sale badge type
$badge = yanka_get_option( 'wc-badge-type' );
...
if ( $product->is_on_sale() || ! $product->is_in_stock() ) : ?>
	<span class="badge pa tc dib <?php echo 'position-' . implode( ' ', $classes ); ?>">
		...
		if ( $product->is_on_sale() && $product->is_in_stock() ) {
			if ( $badge == 'text' ) {
				echo apply_filters( 'woocommerce_sale_flash', '<span class="onsale">' . esc_html__( 'Sale', 'yanka' ) . '</span>', $post, $product );
			} else {
		...
		if ( ! $product->is_in_stock() ) {
			echo '<span class="sold-out">' . esc_html__( 'Out Of Stock', 'yanka' ) . '</span>';
		}
		?>
	</span>
<?php endif; ?>
