...
$product_style       = yanka_get_option('wc-product-style', 1);
$product_style_thumb = yanka_get_option('wc-product-style-thumb', '1');
$product_swatches    = yanka_get_option('wc-attribute-variation', 1);
$product_thumb_hover = yanka_get_option('wc-product-image-hover', 1);
$product_spacing     = yanka_get_option('wc-gutter-space', 40);
$product_load_effect = yanka_get_option('wc-product-style-load', 'rollIn');
// Get product options
...
<div <?php post_class( $classes ); ?>>
	<?php 
	if (isset($woocommerce_loop['type_product']) && $woocommerce_loop['type_product'] == 'grid' && $woocommerce_loop['style_product'] !== ''){
		wc_get_template_part( 'content-box/content', 'product-' . $woocommerce_loop['style_product'] ); 
	}elseif (isset($woocommerce_loop['type_product']) && $woocommerce_loop['type_product'] == 'list') {
