...
(function($) {
    ...
    var samplePage = function( $form ) {
        ...
        this.mainArea = $('.yanka-import-area');

        this.responseArea = this.form.find('.import-response');

        this.progressBar = this.form.find('.yanka-import-progress');
    ...
    samplePage.prototype.formSubmit = function(e) {
        ...
        if( $form.find('#full_import').prop('checked') == true ) {
            ...
            var allpages = $form.find('.yanka_pages').val();
        ...
        importBox.callImportAJAX( page, function(response) {
            ...
            importBox.progressBar.parent().find('.yanka-notice').remove();
    ...
    samplePage.prototype.callImportAJAX = function( page, success, complete ) {
        ...
        $.ajax({
            ...
            error: function( response ) {
                box.responseArea.html( '<div class="yanka-warning">Import AJAX problem. Please try import again.</div>' ).fadeIn();
    ...
    samplePage.prototype.handleResponse = function( response ) {
        ...
        if( ! response ) {
            this.responseArea.html( '<div class="yanka-warning">Empty AJAX response, please try again.</div>' ).fadeIn();
        } else if( rJSON.status == 'success' ) {
            console.log(rJSON.message);
            this.responseArea.html( '<div class="yanka-success">All data imported successfully!</div>' ).fadeIn();
        } else if( rJSON.status == 'fail' ) {
            this.responseArea.html( '<div class="yanka-error">' + rJSON.message + '</div>' ).fadeIn();
        } else {
    ...
    samplePage.prototype.fakeLoading = function(fake1progress, fake2progress, noticeProgress) {
        ...
        this.noticeTimeout = setTimeout( function() {
            ...
            that.progressBar.after( '<p class="yanka-notice small">Please, wait. Theme needs much time to download all attachments</p>' );
        }, 60000 );

        this.errorTimeout = setTimeout( function() {
            that.progressBar.parent().find('.yanka-notice').remove();
            that.progressBar.after( '<p class="yanka-notice small">Something wrong with import. Please, try to import data manually</p>' );
        }, 3100000 );
    };
    ...
    jmsImportAdmin = (function() {
        ...
        var importAdmin = {
            importAction: function() {
                $('.yanka-import-form').each(function() {
                    $(this).import_box();
                })
            },
        };
