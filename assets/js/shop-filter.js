(function( $ ) {
		...
		if (currentUrl) {
			...
			$.ajax({
				...
				success: function (response) {
					...
					if($(response).find('.yanka-ajax-loadmore').length > 0) {
						$( '.yanka-ajax-loadmore' ).html($(response).find('.yanka-ajax-loadmore').html());
					} else {
						$( '.yanka-ajax-loadmore' ).empty();
					}
	...
	$(document).on('click', yankaTheme.ajaxLinks, function (e) {
