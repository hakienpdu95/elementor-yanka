...
$portfolio_limit      = yanka_get_option( 'portfolio-number-per-page', '12' );
$portfolio_style      = yanka_get_option( 'portfolio-style', 'default' );
$portfolio_spacing    = yanka_get_option( 'portfolio-spacing', '40' );
$portfolio_columns    = yanka_get_option( 'portfolio-columns', '3' );
$portfolio_fullwidth  = yanka_get_option( 'portfolio-fullwidth', 0 );
$portfolio_pagination = yanka_get_option( 'portfolio-pagination-type', 'number' );
		...
		<div class="<?php echo esc_attr( $portfolio_container_class ); ?>">
			<?php if ( ! is_tax( 'portfolio-cat' ) ) : ?>
				<div class="portfolio-filter tc mb_32">
					<a data-filter="*" class="selected dib" href="javascript:void(0);"><?php esc_html_e( 'All', 'yanka' ); ?></a>
			...
			<?php if ( $portfolio_pagination == 'loadmore' || $portfolio_pagination == 'infinite' ) : ?>
				<div class="yanka-ajax-loadmore tc mt_20" data-load-more='{"page":"<?php echo esc_attr( $wp_query->max_num_pages ); ?>","container":"portfolio-layout","layout":"<?php echo esc_attr( $portfolio_pagination ); ?>"}'>
					<?php echo next_posts_link( esc_html__( 'Load More', 'yanka' ) ); ?>
				</div>
			<?php else : ?>
				<?php yanka_post_pagination(); ?>
			<?php endif; ?>
		</div>
	</div>
</div>
