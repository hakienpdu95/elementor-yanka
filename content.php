...
$blog_style   = yanka_get_option( 'blog-style', 'flat' );
$blog_columns = yanka_get_option( 'blog-columns', 3 );
$blog_image_size = yanka_get_option('blog-image-size', '1080x570');
					...
					<div class="post-img-wrap">
						<a href="<?php echo esc_url( get_permalink() ); ?>">
							...
							if ( isset( $blog_image_size ) && $blog_image_size != '' ) {
								echo yanka_get_post_thumbnail($blog_image_size);
							} else {
			...
			<?php else : ?>
				<div class="entry-content yanka-entry-content">
					...
					if( isset( $blog_design ) && $blog_design == 'default' && ! class_exists('Redux') ) {
						...
						yanka_get_content(false);
					}
					?>
					
					<?php wp_link_pages( array( 'before' => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'yanka' ) . '</span>', 'after' => '</div>', 'link_before' => '<span>', 'link_after' => '</span>' ) ); ?>
				</div><!-- .entry-content -->
			<?php endif; ?>
